...
begin
  ...
  BinarySensor.SendConfig;
  Button.SendConfig;
  Cover.SendConfig;
  DeviceTrigger.SendConfig;
  Fan.SendConfig;
  Light.SendConfig;
  Lock.SendConfig;
  Sensor.SendConfig;
...
begin
  ...
  did := 'demo_device_' + Format('%3.3d', [cfgSerialNumber]); //Device ID
  MyDevice := TMQTTDevice.Create;
  MyDevice.Config[CDeviceNames[dnDevice_Identifiers]] := did;
  MyDevice.Config[CDeviceNames[dnDevice_Manufacturer]] := 'NicolaPerotto';
  MyDevice.Config[CDeviceNames[dnDevice_Model]] := 'Lazarus/Fpc';
  MyDevice.Config[CDeviceNames[dnDevice_Name]] := 'Demo Device x64';
  MyDevice.Config[CDeviceNames[dnDevice_SWVersion]] := '2024.12.1';
  ab.Host := cfgMqttHost;
  ab.Port := cfgMqttPort;
  ab.ID   := cfgMqttID;
  ab.User := cfgMqttUser;
  ...
  with MyDevice do begin
    ...
    Broker := ab;
{
    Broker.Host := cfgMqttHost;
    Broker.Port := cfgMqttPort;
    Broker.ID   := cfgMqttID;
    Broker.User := cfgMqttUser;
  ...
  with BinarySensor do begin
    ...
    Config[CBinarySensorNames[bsnQos]] := '1'; //maximum usable
    QoS := 1; //use this now because it's a demo!
    Retain := False;
  ...
  with Button do begin
    ...
    //Config[CButtonNames[bnIcon]] := 'mdi:speedometer'; //not necessary if DeviceClass is used (and you like the default)
  ...
  with Lock do begin
    ...
    //@@@ learn how to use lonCodeFormat
    QoS := 0;
    Retain := False;
    State := False;
    OnReadData := LockData;
  ...
  with Switch do begin
    ...
    QoS := 1; //use this for testing the demo
    Retain := False;
    State := False;
...
begin
  ...
  try
    ...
    cfgMqttHost := Ini.ReadString('server', 'host', cfgMqttHost);
    cfgMqttPort := Ini.ReadInteger('server', 'port', cfgMqttPort);
    cfgMqttID   := Ini.ReadString('server', 'id', cfgMqttID);
    cfgMqttUser := Ini.ReadString('server', 'user', cfgMqttUser);
    cfgMqttPass := Ini.ReadString('server', 'pass', cfgMqttPass);
    cfgMqttSSL  := Ini.ReadBool('server', 'ssl', cfgMqttSSL);
  finally
    FreeAndNil(Ini);
  end;
  CoverInternalModify := False;
